...
void BancorNetwork::on_clear(const clear& act) {
    require_auth(_self);
...
void BancorNetwork::on_reg_converter(const regconverter& sc) {
    require_auth(_self);
...
void BancorNetwork::on_reg_token(const regtoken& sc) {
    require_auth(_self);
...
void BancorNetwork::apply(account_name contract, account_name act) {
    ...
    if (name{act} == N(regtoken)) 
        on_reg_token(unpack_action_data<regtoken>());
    else if (name{act} == N(regconverter)) 
        on_reg_converter(unpack_action_data<regconverter>());
    else if (name{act} == N(clear))
        on_clear(unpack_action_data<clear>());
}
