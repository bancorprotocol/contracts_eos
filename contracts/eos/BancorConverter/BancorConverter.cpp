...
ACTION BancorConverter::init(name     smart_contract,
    ...
    if (extend.has_value()) {
        extend_t x = extend.value();
        check(is_account(x.stake_contract), "invalid stake account");
...
ACTION BancorConverter::update(bool smart_enabled, bool enabled, 
    ...
    if (st.extend.has_value()) {
        extend_t x = st.extend.value();
        if (is_account(x.stake_contract)) {
            require_auth(x.stake_contract);
            authorized = true;
        }
    } if (!authorized)  require_auth(get_self());
...
ACTION BancorConverter::setreserve(name contract, asset currency, uint64_t ratio, bool p_enabled) {
    ...
    auto reserve_balance = ((get_balance_amount(contract, get_self(), currency.symbol.code())) + currency.amount) / pow(10, currency.symbol.precision()); 
