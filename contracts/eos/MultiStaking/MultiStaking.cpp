...
void MultiStaking::deposit(name from, name to, asset quantity, string memo) {
  if (from != get_self()) {
    configs configs_table(get_self(), get_self().value);
    ...
    check(st.multi_token.value == _first_receiver.value, "multi-token mismatch");
...
extern "C" void apply(uint64_t receiver, uint64_t code, uint64_t action) {
    if (action == "transfer"_n.value && code != receiver) 
        ...
        eosio::name(code), &MultiStaking::deposit);
    if (code == receiver)
        switch (action) { 
            EOSIO_DISPATCH_HELPER(MultiStaking, (setconfig)(delegate)(undelegate)(refund)(vote))
        }          
}
