...
#include "../MultiToken/MultiToken.hpp"
...
ACTION MultiConverter::setreserve(symbol_code converter_currency_code, asset currency, name contract, bool sale_enabled, uint64_t ratio) {
    ...
    _setreserve(converter_currency_code, converter.owner, currency.symbol, contract, sale_enabled, ratio, smart_supply.amount); 
}
...
ACTION MultiConverter::withdraw(name owner, asset quantity, symbol_code converter_currency_code) {
    require_auth(owner);
    check(quantity.is_valid() && quantity.amount > 0, "invalid quantity");
    modreserve(owner, -quantity, converter_currency_code, get_self(), true);
}
...
void MultiConverter::modreserve(name owner, asset quantity, symbol_code converter_currency_code, name code, bool setup) {
    ...
    converters converters_table(get_self(), converter_currency_code.raw());
    const auto& converter = converters_table.get(converter_currency_code.raw(), "converter does not exist");
    check((!setup && converter.enabled) || !converter.launched, "converter was launched, or must be enabled");
...
void MultiConverter::_setreserve(symbol_code converter_currency_code, name owner, symbol currency, name contract, bool sale_enabled, uint64_t ratio, uint64_t smart_token_supply) {
    string error = "ratio must be between 1 and " + std::to_string(MAX_RATIO);
    ...
    if (reserve != reserves_table.end()) {
        ...
        EMIT_PRICE_DATA_EVENT(converter_currency_code, 
                              smart_token_supply / pow(10, currency.precision()), 
                              reserve->contract, currency.code(), reserve_balance, ratio);
    } else 
        reserves_table.emplace(owner, [&](auto& r) {
            r.contract  = contract;
            r.ratio     = ratio;
            r.sale_enabled = sale_enabled;
...
void MultiConverter::verify_entry(name account, name currency_contract, symbol currency) {
    MultiToken::accounts accountstable(currency_contract, account.value);
...
asset MultiConverter::get_supply(name contract, symbol_code sym) {
    MultiToken::stats statstable(contract, sym.raw());
...
void MultiConverter::transfer(name from, name to, asset quantity, string memo) {    
     // avoid unstaking and system contract ops mishaps
    if (from == get_self() || to != get_self() || from == "eosio.ram"_n || from == "eosio.stake"_n)
        return;
    
    const auto& splitted_memo = split(memo, ";"); 
    
    if (splitted_memo[0] == "setup")
        modreserve(from, quantity, symbol_code(splitted_memo[1]), _first_receiver, true);
    else if (splitted_memo[0] == "fund")
        modreserve(from, quantity, symbol_code(splitted_memo[1]), _first_receiver, false);
    else if (splitted_memo[0] == "liquidate")
        liquidate(from, quantity);
    
    else convert(from, quantity, memo, _first_receiver); 
}

extern "C" void apply(uint64_t receiver, uint64_t code, uint64_t action) {
    if (action == "transfer"_n.value && code != receiver) {
        eosio::execute_action(eosio::name(receiver), eosio::name(code), &MultiConverter::transfer);
    }
    if (code == receiver) {
        switch (action) {
