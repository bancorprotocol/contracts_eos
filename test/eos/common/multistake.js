...
const delegate = async function (amount = defaultAmt, symbol = bntRelaySymbol,
...
const undelegate = async function (amount = defaultAmt, symbol = bntRelaySymbol,
                                   from = user1, to = from, unstake = false, auth = to) {
    try {
        const result = await api.transact({ 
            actions: [{
                account: multiStaking,    
...
const getDelegate = async function (delegator = user1, delegate = user1,
                                    symbol = bntRelaySymbol) {
...
const getDelegates = async function (delegator = user1, symbol = bntRelaySymbol) {
    const symbolHexLE = getTableBoundsForSymbol(symbol).lower_bound;
    
    const bounds = {
        lower_bound: `0x0000000000000000${symbolHexLE}`,
        upper_bound: `0xFFFFFFFFFFFFFFFF${symbolHexLE}`,
    }
    try {
        const result = await rpc.get_table_rows({
            "code" : multiStaking,
...
module.exports = {
    vote, refund, delegate, undelegate, setConfig,
    getVotes, getDelegates, getWeights,
    getRefunds, getUserStake, getDelegate
}
