...
void BancorConverter::on_create(const create& sc, account_name code) {
    require_auth(_self);
...
void BancorConverter::on_set_connector(const BancorConverter::setconnector& sc, account_name code) {
    require_auth(_self);   
...
void BancorConverter::apply(account_name contract, account_name act) {
    ...
    if (name{act} == N(create)) {
        on_create(unpack_action_data<create>(), contract);
    }
    else if (name{act} == N(setconnector)) {
        on_set_connector(unpack_action_data<setconnector>(), contract);
    };
}
